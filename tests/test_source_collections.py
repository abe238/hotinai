from hotin.sources import collections_ai
...
def test_parses_collections_ranking_into_repo_records():
    records = collections_ai.parse_ranking(RANKING, "AI Agents")
    assert records == [{
        "entity_type": "repo",
        ...
        "signal": {"stars": 120, "stars_growth": 14, "collections_ai_rank": 2},
        "meta": {"trend_collection": "AI Agents", "on_trending_list": True},
    }]


def test_filters_non_ai_collection_names():
    assert collections_ai.parse_collections(COLLECTIONS) == [
        (10, "AI Agents"), (11, "Machine Learning")
    ]


def test_dedupes_by_entity_id_with_highest_stars_growth():
    lower = collections_ai.parse_ranking(RANKING, "AI Agents")
    higher = collections_ai.parse_ranking(
        {"rows": [{"repo_name": "owner/project", "total": 130, "current_period_growth": 30,
                    "current_period_rank": 1}]}, "LLM tools"
    )
    records = collections_ai.dedupe_records(lower + higher)
    assert len(records) == 1
    assert records[0]["signal"]["stars_growth"] == 30
    assert records[0]["meta"]["trend_collection"] == "LLM tools"
...
def test_hostile_inputs_degrade_without_raising():
    ...
    records = collections_ai.parse_ranking(hostile, "AI")
    assert len(records) == 1
    assert records[0]["signal"] == {"stars": 0, "stars_growth": 0, "collections_ai_rank": None}
    assert collections_ai.parse_collections(None) == []
    assert collections_ai.parse_ranking(None, "AI") == []


def test_fetch_errors_when_all_requests_fail(monkeypatch):
    monkeypatch.setattr(collections_ai, "_request", lambda url: None)
    result = collections_ai.fetch(limit=5)
    assert result["status"] == "error"
    assert result["records"] == []


def test_selftest():
    collections_ai.selftest()
